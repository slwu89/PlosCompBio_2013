...
estimateLarvalEquilib = function()
{
  ...
  Smf = rep(1, Nf) # Smf: susceptible mosquitoes at feeding sites
  ...
  for (tt in 1 : T)
  {
    ...
    # Sml = Emerging Larvae + Incoming Migration from Feeding Sites
    Sml = M_larvae[,xi] + rmultinomMatrix(Smf, L) # uninfected

    # Smf: susceptible mosquitoes at blood feeding sites
    # Smf = Incoming Migration from Laying Sites (includes mosquitoes who just emerged)
...
################################################################################
# Matrix of time at risk (if rows sum < 1 it just means they aren't at risk for all of that day)
# dim: humans x sites (says how i distribute my time)
################################################################################
...
################################################################################
# How bites get put onto hosts at a site.
# dim: humans x site (says if I'm a mosquito at site = column j, how do i choose who to bite)
################################################################################
...
################################################################################
# Just a movement matrix with mortality
################################################################################
...
# A proportion (1 - surv) perish.
{
  ...
  # Returns a movement vector from (x[i], y[i]) to all (X, Y)
  {
    ...
    if(mixing == 'poor')
    {
      m = surv * normalize(dnorm(dd, 0, .025 * 2 * sqrt(1 / pi)))
    } # end if
    if(mixing == 'well')
    {
